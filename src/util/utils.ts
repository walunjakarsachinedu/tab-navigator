...
function fuzzySearch(tabData: TabData[], searchTerm: string): (TabData & { match: Match })[] {
  return tabData
    .map<TabData & { match: Match }>(tab => {
      ...
      return {
        ...
        match: {
          ...
          url: urlMatchIndices,
          rank: _calculateRank({title: titleMatchIndices, url: urlMatchIndices})
        }
      };
    })
    .filter(tab => (tab.match.title.length + tab.match.url.length) == searchTerm.length)
...
export { getCurrentWindowId, fuzzySearch, removeSchemaFromUrl };