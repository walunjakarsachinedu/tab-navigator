...
class TabTracker {
  ...
  constructor() {
    ...
    this._isLoading = this.restoreState().finally(() =>
  ...
  async getTabQue(): Promise<TabData[]> {
    ...
    return this._tabQue
      .getQueData()
      .filter((tabs) => tabs.windowId == windowId);
  }
  ...
  private _onTabActivated(activeInfo: chrome.tabs.TabActiveInfo) {
    this._tabQue.moveFront({
      getData: TabTracker.getTabByIdFunc(activeInfo.tabId),
    });
    this.storeState();
  }
      ...
      return new Promise((resolve) => {
        chrome.tabs.query({}, async (tabs) => {
          ...
          const remainingTabs = tabs
            .filter((tab) => tabData.every((t) => t.id != tab.id))
            .map<TabData>((tab) => ({
              id: tab.id!,
              url: tab.url ?? "",
              title: tab.title ?? "",
  ...
  static getTabByIdFunc(tabId: number): DataGetter<TabData> {
    return (que) => {
      return que.find((el) => el.id === tabId);
    };
  }
}
